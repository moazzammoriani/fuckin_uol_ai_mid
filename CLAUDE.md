...
```bash
# Recommended: run.py with CLI arguments
cd src && python run.py --help
cd src && python run.py --generations 1000 --pop-size 50
...
- **simulation.py**: `Simulation` runs headless PyBullet physics with configurable environments via `Environment` enum (PLANE or GAUSSIAN_PYRAMID). `ThreadedSim` uses a persistent multiprocessing Pool for parallel evaluation. URDF files are written to `/dev/shm` (RAM) and deleted after loading.

- **run.py**: Main GA runner with CLI arguments for all hyperparameters. Saves outputs to organized `data/run_<timestamp>/` directories.
...
The simulation computes fitness as average proximity to the mountain peak:
```
fitness = 1 / (1 + avg_distance_to_peak)
```
- Peak location: `(0.4, 0, 4.1)`
- Uses average distance over all simulation steps (rewards sustained proximity, not just touching the peak once)
- If creature falls below terrain (z < -0.5), fitness = 0

### Evolution Loop Pattern

The GA follows a standard pattern:
...
4. Preserve elite individual
5. Call `sim.close()` when done to clean up the process pool

### DNA Persistence

Genomes serialize to/from CSV files using `Genome.to_csv()` and `Genome.from_csv()`.

### Output Structure

When using `run.py`, outputs are saved to `data/run_<timestamp>/`:
- `elites/elite_N.csv`: Best creature DNA at generation N
- `fitness_plot.png`: Best/mean fitness over generations
- `generations.csv`: Per-generation fitness data
- `hyperparameters.json`: All GA settings for reproducibility
